              ...
              <ul className="shop-category">
                {menuData.map((menuItems, index) => (
                  ...
                  <li
                    ...
                    onMouseEnter={() => setLiIndex(index)}
                    key={`${menuItems.categoryName}+${index}`}
                  >
                    {menuItems.categoryName !== '' && (
                      <Link key={`${menuItems.categoryName}+${index}`}>{menuItems.categoryName}</Link>
                    )}
                      ...
                      {menuItems?.data.map((subMenuItems, index) => (
                        <ul>
                          ...
                          {subMenuItems?.subMenu.map((menu, index) => (
                            <Link
                              to={
                                JSON.stringify(menu).toLowerCase().includes('View All'.toLowerCase())
                                  ? menu.path
                                  : menu.path + addDash(menu.name)
                              }
