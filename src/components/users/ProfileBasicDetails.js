...
export default function ProfileBasicDetails() {
    ...
    const validate = ()=>{
        if(userInfo?.full_name !== '' && userInfo?.dob !== '' && userInfo?.social_category !== '' && userInfo?.gender !== '' && userInfo?.marital_status !== '' &&
        userInfo?.physically_challenged !== '' && userInfo?.city !== '' && userInfo?.state !== ''){
            setDisable(false)
        }else{
            setDisable(true)
        }
    ...
    const handleSave = async ()=>{
        ...
        const response = await dispatch(saveUserBasicDetails({
    ...
    useEffect(()=>{
        validate()
    },[userInfo?.full_name, userInfo?.dob, userInfo?.social_category, userInfo?.gender, userInfo?.marital_status, userInfo?.physically_challenged, userInfo?.city, userInfo?.state])
    useEffect(()=>{
        fetchUserBasicDetail()
    },[])
  return (
            ...
            <ProfileTheme>
                ...
                <div className="fname-dob-social-hide">
                    ...
                    <CustomSelectBox
                        ...
                        name="test"
                        arialInvalid="false"
                        onChange={(e)=>dispatch(updateUserInfo({userData:{...userInfo, social_category:e.target.value}}))}
                        style={{fontWeight:"600"}}
                    ...
                    <CustomeInputRadio 
                        ...
                        name={'frequency1'}
                        onChange={(e)=>dispatch(updateUserInfo({userData:{...userInfo, marital_status:e.target.value}}))}
                    ...
                    <CustomeInputRadio 
                        ...
                        options={['Yes', 'No']}
                        name={'frequency2'}
                        onChange={(e)=>dispatch(updateUserInfo({userData:{...userInfo, physically_challenged:e.target.value}}))}
